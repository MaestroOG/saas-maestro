                ...
                <div className='companion-section'>
                    <div className='companion-avatar' style={{ backgroundColor: getSubjectColor(subject) }}>
                        ...
                        <div className={cn('absolute transition-opacity duration-1000', callStatus === CallStatus.ACTIVE ? 'opacity-100' : 'opacity-0')}>
                            <Lottie animationData={soundwaves} lottieRef={lottieRef} autoPlay={false} className='companion-lottie' />
                        </div>
                    </div>
                ...
                <div className='user-section'>
                    ...
                    <button className='btn-mic' onClick={toggleMicrophone} disabled={callStatus !== CallStatus.ACTIVE}>
                        ...
                        <p className='max-sm:hidden '>
                            {isMuted ? 'Turn on microphone' : 'Turn on microphone'}
                        </p>
                    </button>
                ...
                <div className='transcript-message no-scrollbar'>
                    {messages.map((message, index) => {
                        if (message.role === 'assistant') {
                            ...
                            return (
                                <p className='text-primary max-sm:text-sm' key={index}>{userName}: {message.content}</p>
                            )
                        }
                    })}
                </div>
